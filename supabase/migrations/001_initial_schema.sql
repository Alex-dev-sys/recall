-- ==========================================
-- RECALL PROJECT - DATABASE SCHEMA
-- ==========================================
...
-- ==========================================
-- TABLE: profiles
-- Stores user profile data and Telegram session info
-- ==========================================
...
-- Index for faster telegram user lookups
...
-- Stores chats that user wants to monitor
-- ==========================================
...
CREATE INDEX idx_monitored_chats_user_id ON public.monitored_chats(user_id);
CREATE INDEX idx_monitored_chats_chat_id ON public.monitored_chats(chat_id);
CREATE INDEX idx_monitored_chats_active ON public.monitored_chats(user_id, is_active) WHERE is_active = true;
...
CREATE TABLE IF NOT EXISTS public.tasks (
    ...
    FOREIGN KEY (user_id, chat_id) REFERENCES public.monitored_chats(user_id, chat_id) ON DELETE CASCADE
);

-- Indexes for queries
CREATE INDEX idx_tasks_user_id ON public.tasks(user_id);
CREATE INDEX idx_tasks_status ON public.tasks(user_id, status) WHERE status NOT IN ('done', 'cancelled');
CREATE INDEX idx_tasks_priority ON public.tasks(user_id, priority, deadline);
CREATE INDEX idx_tasks_deadline ON public.tasks(deadline) WHERE deadline IS NOT NULL AND status NOT IN ('done', 'cancelled');
CREATE INDEX idx_tasks_created_at ON public.tasks(created_at DESC);
CREATE INDEX idx_tasks_chat_id ON public.tasks(chat_id, message_id);
...
-- Tracks active worker sessions (for monitoring)
-- ==========================================
...
CREATE INDEX idx_worker_sessions_user_id ON public.worker_sessions(user_id);
CREATE INDEX idx_worker_sessions_status ON public.worker_sessions(status) WHERE status IN ('running', 'error');
...
-- Function: Auto-set completed_at when status changes to done
...
-- Monitored chats: Users can only manage their own chats
...
-- Task notifications: Read-only for users
...
-- Worker sessions: Read-only for users
CREATE POLICY "Users can view own worker sessions" ON public.worker_sessions
    FOR SELECT USING (auth.uid() = user_id);

-- ==========================================
-- SERVICE ROLE POLICIES (for backend worker)
...
-- View: Active tasks with priority ordering
